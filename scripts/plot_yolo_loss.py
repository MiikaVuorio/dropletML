import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import os

# =========================================================================
# === CONFIGURATION =======================================================
# =========================================================================

# --- Input File Paths ---
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
RESULTS_CSV_PATH = os.path.join(BASE_DIR, "..", "runs", "yolov5_runs", "train","droplet_detection_run", "results.csv")

# --- Output File Path ---
OUTPUT_PLOT_PATH = os.path.join(BASE_DIR, "..","data", "plots", "yolo_training_loss_curve.png")

# --- Plotting Settings ---
PLOT_TITLE = 'Training and Validation Loss Over Epochs'
X_AXIS_LABEL = 'Epoch'
Y_AXIS_LABEL = 'Location Loss'
# =========================================================================


def create_loss_plot():
    """
    Loads training and validation loss data from TensorBoard CSVs
    and creates a publication-quality plot.
    """
    print("Loading data...")
    try:
        train_df = pd.read_csv(RESULTS_CSV_PATH,skipinitialspace=True)
    except FileNotFoundError as e:
        print(f"FATAL ERROR: Could not find data files. Make sure they are in the project root.")
        print(f"Missing file: {e.filename}")
        return

    print("Creating plot...")
    
    sns.set_theme(style="whitegrid")
    
    fig, ax = plt.subplots(figsize=(12, 7))
    
    ax.plot(train_df['epoch'], train_df['train/box_loss'], label='Training Loss', color='blue', linewidth=2)
    ax.plot(train_df['epoch'], train_df['val/box_loss'], label='Validation Loss', color='orange', linewidth=2)
    
    ax.set_title(PLOT_TITLE, fontsize=16, weight='bold')
    ax.set_xlabel(X_AXIS_LABEL, fontsize=12)
    ax.set_ylabel(Y_AXIS_LABEL, fontsize=12)
    ax.legend(fontsize=12)
    
    # ax.set_yscale('log')
    
    # Set limits to zoom in on the interesting part of the graph if needed
    # For example, to ignore the initial high loss from the first few epochs
    # ax.set_ylim(0, 70) 
    
    # Ensure the layout is tight to prevent labels from being cut off
    plt.tight_layout()
    
    # Save the figure with high resolution, suitable for a thesis
    plt.savefig(OUTPUT_PLOT_PATH, dpi=300)
    
    print(f"\nPlot saved successfully to: {OUTPUT_PLOT_PATH}")

    # Print the best epoch
    best_epoch_step = train_df.loc[train_df['val/box_loss'].idxmin()]['epoch']
    print("Epoch with the lowest validation loss: " + str(best_epoch_step))
    
    # Display the plot in a pop-up window for immediate viewing
    plt.show()

if __name__ == "__main__":
    create_loss_plot()